...
function drawMainMenu() {
  if (!menuStaticDrawn || menuSelection !== menuLastSelection) {
    ...
    setTextSize(1);
    var quitText = menuOptions[menuOptions.length - 1];
    var quitWidth = quitText.length * 6;
    var quitX = (WIDTH - quitWidth) / 2;
    var quitY = frameY + frameHeight + 20;

    if (menuSelection === menuOptions.length - 1) {
      setTextColor(YELLOW);
      drawString("> " + quitText, quitX - 10, quitY);
    } else {
      setTextColor(WHITE);
      drawString("  " + quitText, quitX, quitY);
    }
